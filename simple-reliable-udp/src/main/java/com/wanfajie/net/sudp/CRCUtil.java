...
public class CRCUtil {
    ...
    private CRCUtil() { throw new UnsupportedOperationException(); }
    ...
    public static byte crc8(byte[] data, int offset, int len, byte preval) {
        byte ret = preval;
        int bound = offset + len;
        for (int i = offset; i < bound; ++i) {
            ret = crc8Table[0xff & (ret ^ data[i])];
        }
        return ret;
    }

    public static byte crc8(ByteBuf buf) {
        return crc8(buf, buf.readerIndex(), buf.readableBytes(), (byte) 0);
    }

    public static byte crc8(ByteBuf buf, int offset, int len) {
        return crc8(buf, offset, len, (byte) 0);
    }

    public static byte crc8(ByteBuf buf, int offset, int len, byte preval) {
        byte ret = preval;
        int bound = offset + len;
        for (int i = offset; i < bound; i++) {
            ret = crc8Table[0xff & (ret ^ buf.getByte(i))];
        }
        return ret;
    }
